...
fn main() -> io::Result<()> {
    ...
    let mods =
        TextOpsMod_Ref::load_from_directory(&library_path).unwrap_or_else(|e| panic!("{}", e));

    let opts = Command::clap().get_matches().piped_ref(Command::from_clap);
    ...
    match opts {
        Command::ReverseLineOrder => {
            ...
            let reversed = mods.reverse_lines()(&mut state, buffer.as_str().into());
        ...
        Command::Json { file } => {
            fn run_command(mods: TextOpsMod_Ref, state: &mut TOStateBox, s: &str) -> RString {
                let command = serde_json::from_str(s).unwrap_or_else(|e| panic!("\n{}\n", e));
